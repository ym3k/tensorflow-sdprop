...
/*
class ResourceApplySDProp : public XlaOpKernel {
 public:
  explicit ResourceApplySDProp(OpKernelConstruction* ctx) : XlaOpKernel(ctx) {}
  ...
  void Compile(XlaOpKernelContext* ctx) override {
    ...
    OP_REQUIRES(
        ctx, type == var_type && type == ms_type && type == mom_type,
        errors::InvalidArgument(
            "Types of variable arguments to ResourceApplySDProp must match: ",
            DataTypeString(type), " vs. ", DataTypeString(var_type), " vs. ",
            DataTypeString(ms_type), " vs. ", DataTypeString(mom_type)));
...
REGISTER_XLA_OP(Name("ResourceApplySDProp"), ResourceApplySDProp);
*/
}  // namespace
}  // namespace tensorflow
